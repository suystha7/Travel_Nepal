...
export async function generateMetadata() {
  try {
    const response = await getStaticData<ISeoResponse>(endpoints.SEO.RESERVATION);
...
const Page = async () => {
  ...
  const contactData = results[0].status === "fulfilled" ? results[0].value : null;
  const orgResponse = results[1].status === "fulfilled" ? results[1].value : null;
  const reservationData = results[2].status === "fulfilled" ? results[2].value : null;
